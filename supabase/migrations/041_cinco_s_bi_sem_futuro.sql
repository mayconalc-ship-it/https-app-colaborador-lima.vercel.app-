-- ==================================================================
-- 041 - BI 5S IGNORA MES FUTURO
-- Execute no Supabase: SQL Editor > New query > colar > Ctrl+A > Run
-- ==================================================================
-- Depois que o ciclo passou a ser planejado o ano inteiro de uma vez, o
-- BI ganhou meses que ainda nao comecaram: dezembro aparecia no seletor
-- de periodo em agosto, e as auditorias dele entravam na conta de
-- pendentes -- que passou a somar o plano do ano em vez do que esta
-- faltando fazer.
--
-- Auditoria futura continua existindo e aparecendo onde faz sentido: na
-- tela de planejamento do gestor. O que ela nao faz mais e contaminar
-- indicador de execucao.
--
-- Substitui as duas funcoes da 036. O indice criado la continua valendo.
...
base as (
  select a.*
   ...
   where a.revenda_id = p_revenda
     ...
     -- Mes futuro fica de fora. Com o ciclo do ano planejado de uma
     -- vez, dezembro ja existe como linha em agosto -- e conta-la
     -- faria "pendentes" somar o plano inteiro do ano, quando a
     -- pergunta que o cartao responde e "o que falta fazer AGORA".
...
as $$
  select distinct to_char(a.competencia, 'YYYY-MM')
   ...
   where a.revenda_id = p_revenda
     and a.status <> 'cancelada'
     -- Nao oferece mes que ainda nao comecou: nao ha auditoria feita
     -- la, e a pessoa escolheria um filtro que so devolve tela vazia.
     and a.competencia <= date_trunc('month', current_date)::date
   order by 1 desc;
$$;
